(function () {
    ...
    function kGetBooks() {
        ...
        return Promise
            ...
            .then(json => {
                var order = 0;
                JSON.parse(json).books.forEach(book => {
                    kBookList.push({ 'title': book.title, 'tag': book.tag, 'id': 'book-' + order });
                    order++;
                });
                return kBookList;
            });
    ...
    /** 搜索book */
    function kSearchBook(text) {

        var bookSearched = [];
        kBookList.forEach(book => {
            if (book.title.includes(text)) {
                bookSearched.push(
                    {
                        'title': book.title,
                        'content': book.title.replaceAll(text, "<em class=\"search-keyword\">" + text + "</em>"),
                        'url': "#/library/" + book.tag + "?id=" + book.id,
                        'score': 1
                    });
            }
        });
    ...
    function doSearch(value) {
        ...
        // console.log("start search kkk=" + value);
        // console.log("start search kkk=" + JSON.stringify(kBookList));
        ...
        var matchs = kSearchBook(value);
        
        var html = '';
        matchs.forEach(function (post) {

            // console.log("matchs post=" + JSON.stringify(post));
